...
export default async function RequirementPage({ params }: { params: Promise<{ id: string; requirementId: string }> }) {
  ...
  const [projectResponse, membershipResponse, linksResponse, tasksResponse, commentsResponse] = await Promise.all([
    db.from("projects").select("title").eq("id", id).eq("workspace_id", requirement.workspace_id).maybeSingle(),
    db.from("workspace_memberships").select("role").eq("workspace_id", requirement.workspace_id).eq("user_id", userId).maybeSingle(),
