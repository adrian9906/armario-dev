...
export default async function TaskPage({ params }: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  const { db, userId, canEdit, canComment, role, project: accessProject } = await requireProjectAccess(id);
  ...
  if (!task) notFound();
  const [projectResponse, membersResponse, projectMembersResponse, checklistResponse, requirementsResponse, linksResponse, commentsResponse] = await Promise.all([
    db.from("projects").select("title").eq("id", id).eq("workspace_id", task.workspace_id).maybeSingle(),
    db.from("workspace_memberships").select("user_id,role").eq("workspace_id", task.workspace_id),
  ...
  const canWork = canEdit || (role === "contributor" && task.assignee_id === userId);
  const explicitIds = new Set((projectMembersResponse.data ?? []).map((member) => member.user_id));
  const members = (membersResponse.data ?? []).filter((member) => canAssignProjectMember(accessProject, member, explicitIds));
