...
export function IdeaFiltersForm({ workspaceId, query, status, kind }: { workspaceId: string; query: string; status: string; kind: string }) {
  return <form method="get" action="/dashboard" className="mb-7 rounded-3xl bg-white p-4 shadow-sm">
    ...
    <FieldGroup className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-end">
      <Field><FieldLabel htmlFor="idea-search" className="sr-only">Buscar ideas</FieldLabel><div className="relative"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-muted-foreground" aria-hidden /><Input id="idea-search" name="q" maxLength={120} defaultValue={query} placeholder="Buscar por título, notas o etiquetas" className="pl-9" /></div></Field>
