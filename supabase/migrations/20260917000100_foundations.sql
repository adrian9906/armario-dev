...
create index ideas_workspace_created_idx
...
-- Running as the migration owner avoids recursive membership RLS evaluation.
...
-- Trusted triggers create a profile and private space for each new account.
...
begin
  chosen_name := left(
    coalesce(nullif(trim(new.raw_user_meta_data ->> 'full_name'), ''),
             nullif(split_part(new.email, '@', 1), ''), 'Creador'),
    120
  );
...
create policy workspaces_create_self on public.workspaces
...
create policy memberships_read_member on public.workspace_memberships
  for select to authenticated using (private.is_workspace_member(workspace_id));

create policy ideas_read_member on public.ideas
  for select to authenticated using (private.is_workspace_member(workspace_id));
create policy ideas_create_editor on public.ideas
  for insert to authenticated
  with check (private.can_edit_workspace(workspace_id) and author_id = (select auth.uid()));
...
create policy projects_read_member on public.projects
  for select to authenticated using (private.is_workspace_member(workspace_id));
create policy projects_create_editor on public.projects
  for insert to authenticated
  with check (private.can_edit_workspace(workspace_id) and creator_id = (select auth.uid()));
